...
const CardUpload: React.FC<ICardUpload> = ({ name, title, onChange, value }) => {
  return (
    <React.Fragment>
      <div
        content='content'
...
const UploadLicenseProofYourIdentity: React.FC<IUploadLicenseProofYourIdentity> = ({
  ...
  const imageFront = images.front ? [URL.createObjectURL(images.front)] : null;
  const imageBack = images.back ? [URL.createObjectURL(images.back)] : null;
  return (
